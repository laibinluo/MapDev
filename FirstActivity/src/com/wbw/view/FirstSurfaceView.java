...
public class FirstSurfaceView extends SurfaceView implements
	...
	/**
	 * 16/24/32,瑕佸啓鐨勫瓧绗︼紝寮�鐨剎,y,鍊嶆暟锛岃姳鎴栫埍蹇�
	 * @param font_kind
	 * @param s
	 * @param stx
	 * @param sty
	...
	// 鑺�type 1涓鸿姳锛�涓虹埍蹇�
	/**
	 * 寮�x,寮�Y鍧愭爣锛屽瓧鐨勫锛岄珮锛屾枃浠跺悕瀛楋紝鏀惧ぇ鍊嶆暟锛宼ype涓哄浘鐗囩绫�
	 * @param stx
	 * @param sty
	 * @param w
	 * @param h
	...
	class showWenZi extends Thread {
		...
		public void run() {
			...
			if(one1.equals("鏅"))
				...
				int bei16 = 8;
				show_font32(one1, onestartx, onestarty, sw, sh, bei16, 2);
			}
	...
	class showheart extends Thread {
		...
		private void run_hua_heart() {
			...
			for (int i = 0; i < maxh  && !isallstop; i++) {
				...
				double a = 13.5 * (16 * Math.pow(Math.sin(b), 3));  //杩欓噷鐨�3.5鍙互鎺у埗澶у皬
				double d = -13.5
